...
Because of this, I had to review and modify the input-handling logic in detail. In particular, I needed to adjust the event handling so that the GUI version could respond not only to touch or mouse-based slide interactions, but also to keyboard arrow key inputs. As a result, the final version supports both interaction methods, while still preserving the keyboard-based controls that were used in the original CLI version.
